...
- When a user asks for a recurring, periodic, or automatically scheduled task, **always use the internal scheduler** (`schedule_job`) — never use system cron (`crontab`, `systemd timers`, etc.) unless the user explicitly asks for system-level cron. For a multi-step pipeline, write the prompt to load the relevant skill and follow its steps, spawning workers for the heavy ones.
