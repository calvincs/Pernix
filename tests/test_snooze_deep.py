...
# ---------------------------------------------------------------------------
# _archive_entries_in_file and _remove_from_index
# ---------------------------------------------------------------------------


def test_archive_entries_in_file(tmp_path, monkeypatch):
    """_archive_entries_in_file adds archived tag to entries."""
    monkeypatch.setattr("config.settings.memory_dir", str(tmp_path / "memories"))
    import time
    ...
    runner = SnoozeRunner()
    runner._archive_entries_in_file(store, "pernix.notes", {epoch})

    # Read the file and verify archived tag was added
    content = store.read_file("pernix.notes")
    assert content is not None
    # archived tag should be present
    assert "@archived" in content


def test_remove_from_index(tmp_path, monkeypatch):
    """_remove_from_index removes entries from FTS5."""
    monkeypatch.setattr("config.settings.memory_dir", str(tmp_path / "memories"))
    import time
    ...
    runner = SnoozeRunner()
    runner._remove_from_index(store, "pernix.notes", {epoch})
    # Should not raise


# ---------------------------------------------------------------------------
# _update_tags_in_markdown
# ---------------------------------------------------------------------------


def test_update_tags_in_markdown(tmp_path, monkeypatch):
    """_update_tags_in_markdown adds tags to an existing entry."""
    monkeypatch.setattr("config.settings.memory_dir", str(tmp_path / "memories"))
    import time
    ...
    runner = SnoozeRunner()
    runner._update_tags_in_markdown(store, "pernix.notes", epoch, ["new_tag", "enriched"])

    content = store.read_file("pernix.notes")
    assert content is not None


def test_update_tags_in_markdown_missing_file(tmp_path, monkeypatch):
    """_update_tags_in_markdown handles missing file gracefully."""
    monkeypatch.setattr("config.settings.memory_dir", str(tmp_path / "memories"))
    from core.memory.store import MemoryStore
    ...
    runner._update_tags_in_markdown(store, "nonexistent.file", 12345, ["tag"])
