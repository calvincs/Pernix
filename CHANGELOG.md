...
- feat(ui): the mobile and tablet pass — the same interface on a phone, a tablet and a monitor, with the device tier decided by two questions instead of one (design notes: `docs/internals/web-client.md`):
  ...
  - **`mobile.css` is now `compact.css` (width) and `touch.css` (pointer)**, each gated on its own `<link media>`, and `isMobile()` is gone in favour of `isCompact()` / `isTouch()`. `tools/ui-gate/` drives the app across seven viewports and asserts the tier contracts, the touch floors and an unchanged desktop layout.
