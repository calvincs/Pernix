...
- feat(learning): every proposal in Self-tuning → Learning now explains itself. The card used to be the database row: a producer name, a rationale one model wrote for another, `create policy` edits and `pm:`/`hypothesis:` ids — and nothing that said whether the item would apply on its own, was waiting for a click, or had been refused by the clock. Each card now carries three plain sentences: **What** (who proposed it — "Dream, the overnight review…" — and the concrete change), **Why** (what was seen, in which session, how many graded turns or tracked facts back it) and **If you do nothing** ("Applies on its own in 22 h unless you reject it first" / "Waits for you. New self-tests never apply on their own" / "Held for you. Its evidence points to nothing the system recorded"), with a colour badge for the same three fates and the raw rationale, edits and evidence folded under *Details*. The text is template-built from the row (`core/adaptive/explain.py`) — no model call, the same row always reads the same — and rides the API as `explanation` on every proposal. **Chat about this** opens a normal session titled after the proposal and sends an opener asking the agent to explain it; the agent reads the row with the new `adaptive_proposals` tool and, only when you tell it to in that conversation, can approve or reject with `adaptive_proposal_decide` (same engine and rollback as the button; both tools are denied to canary, worker and cron sessions). `POST /api/adaptive/proposals/{id}/discuss`.

- fix(adaptive): a proposal that only removes entries is no longer held as "unfounded". The receipt guard keeps the veto clock from turning unbacked prose into permanent policy, but dream's retirement sweep proposes *deleting* a rule whose hypothesis has vanished with a `retired: …` note as its only evidence — so on the reference box proposal #499 sat pending for three days, logged as held every twenty minutes, kept alive by the check meant to stop unbacked rules. Deletes go to the clock; creates and updates are held exactly as before.
...
- fix(workspace): a folder holding a circular symlink can be deleted again. Before removing a directory the Explorer walks it and refuses any link that points outside the workspace; a link pointing at itself raises rather than answering, so the whole delete came back 500 and the folder was stuck for good. Links that cannot be resolved are now passed over — they have no target to escape to, and the delete unlinks them without following — while a link that really does resolve outside the workspace is refused exactly as before.

- feat(spaces): Pernix notices work you keep coming back to and offers to make a space for it — off by default (Settings → Autonomy & idle work → **Space suggestions**; migration v35, guide: `docs/guides/spaces.md`). At idle, at most once a day and only once ten new chats have appeared, one background-model call groups the last 30 days of ordinary chats by the kind of work they are, and a code gate keeps only the substantial ones: five chats over three distinct days, nothing that reads as small talk, nothing already pending or declined, two per scan and five waiting at a time. A survivor lands as a sparkle row under Spaces in the sidebar — *Suggested · Fact checking*, or *3 chats belong in Pernix* — plus one notification. Opening it gives a review sheet: the name and colour, a tick per chat, and, where that kind of work would benefit from a standing instruction, the drafted directive shown as the current default with the addition appended — editable, or droppable with "Use the default instead". Accepting creates the space and its workspace folder, writes only the directive files you kept, and moves the chats you left ticked without touching their recency; **Don't suggest this** declines the grouping and remembers its topic and its chats, so the same habit is not proposed again until you clear it from the **Declined** list in settings; a suggestion you ignore expires after 14 days and can come back. Nothing is created, moved or written without your click — a scan's only side effect is a pending row and a notification. `GET|DELETE /api/space-suggestions`, `GET /api/space-suggestions/{id}`, `POST /api/space-suggestions/{id}/accept|reject`, `POST /api/space-suggestions/scan {dry_run}` (the settings section's **Scan now** preview runs the last one).

- feat(sidebar): a space looks like a space, and the list below it says where it starts. Every open space now draws a thin rail in its own colour down its left edge, from its first row to its "Show all" button, and everything inside — the Today/Yesterday buckets, the sessions, their workers — sits one level in beside it, so a row still reads as "inside YouTube" after its bucket header has scrolled away. The buckets inside a space are sentence case ("Today") where the list's own are uppercase ("TODAY"), and a **Sessions** heading with a hairline now marks where the last space ends and the ordinary list begins. The heading only appears when there is a seam to mark — at least one space above it and at least one session below it. The cost is ten pixels of title width inside a space; nothing nests any deeper than before.

- fix(sidebar): session titles get the width back. The row's five hover controls — copy id, pin, rename, move to space, delete — kept their 24px targets inside the line, which in a 270px sidebar left the title about seven characters and made two conversations impossible to tell apart. They are an overlay now: out of the line, fading in over the title's tail when you point at the row or tab into it, still 24px when you aim at one, and no longer clickable while invisible. A pinned session keeps a small pin in the line, since the toggle that sets it is now hidden at rest.

- fix(sidebar): space names get the width back too. A space header's three controls — new session, space settings, delete space — were laid out in the header whether or not you were pointing at it, so a 270px sidebar showed about seventeen characters of a space's name and two long names read the same. They move into the same kind of overlay the session rows use: the name now gets the full width of the header, and the three controls fade in over its tail when you point at the header or tab into it. Each is a proper 24px target when it appears — they were 12 and 18 pixels wide before, well under the floor every other control was given — and none of them can be clicked while invisible. Tab still reaches all three, in the same order. Under a finger nothing changes: the header keeps its "+" and its overflow button at full size.

- feat(sidebar): the sidebar's width is yours to set. Drag its right edge — anywhere from 200px to whichever is smaller, 520px or 45% of the window — and the width is remembered in that browser and applied before the next load paints. Double-click the edge to put it back. The edge is also a keyboard control: focus it and the arrow keys move it 16px at a time, Home and End go to the ends. Under a finger, and in a window narrow enough for the sidebar to be a drawer, there is no handle and nothing changes.

- feat(timeline): the state machine as a map of its own, and it all moves with the turn. The Graph tab is now Map: the ten states and their thirty-one transitions hand-laid as one SVG, painted straight from the state palette (no colour bridge to break, and no more dark box for idle on the light theme), the edges this session took drawn solid with their counts, the current state lit. Mermaid is gone, and 3.3 MB with it. While a turn runs, the lane's last segment grows and its clock ticks, tool calls land as ticks the moment they start, the map's lit state follows each transition, and the story's Act card fills in call by call, with the server's record replacing the live sketch once the turn ends. Reduced motion keeps every update and drops the pulses.

- feat(timeline): a lane of turns, and the story of the one you pick. The State timeline opens on a new Lane tab: one row per turn, the turn's phases as proportional segments in the state palette, tool calls as ticks with errors in red, the duration at the edge, and a running turn marked as such. Pick a row (click, or arrow keys and Enter) and the Story below tells that turn in the agent's own words, in the order the machine ran it: Plan (the scout's approach, the tools it chose and why, what it recalled), Act (the calls with latency, the tokens, the model), Verify (the reflect verdict and reasoning, the gates with their commands and exit codes), Remembers (compaction summaries and notices). The Timeline list's turn headers open the same story. Data comes from the new per-turn endpoint; the Graph tab is unchanged for now.

- feat(api): one record per turn for the timeline. `GET /api/sessions/{id}/turns` (newest first, `before_turn` paging) assembles, server-side, what the agent produced in each turn: the phases with durations, every tool call with latency and the executor's error flag, the scout report, the reflect chain, the eval gates, compaction summaries, notices and token totals. Two corrections to what the old timeline computed: a turn's elapsed no longer includes the idle time before its prompt arrived, and a tool call counts as an error when the executor said so, not when its output happened to contain the word "traceback".

- feat(sidebar): the list scales. Hiding a type in the legend now changes what the server sends (`GET /api/sessions?exclude_types=canary,…`), so a hidden type no longer eats page slots — on a box where 277 of the first 500 rows were canary runs, page one is now the sessions you actually look at; the legend keeps the hidden type's count from a `type_counts` census. Inside a space, sessions fall into the same Today / Yesterday / This week / This month / Older buckets as outside, "Older" starts folded, and past 15 rows a "Show all N" control folds the long tail.

- feat(sessions): archive, not delete. A chat can be archived from its row (the hover controls on a desktop, the `⋯` sheet on touch) or from a space's actions ("Archive idle sessions…"). An archived chat leaves the sidebar and its space group, keeps every message, still turns up in search with an "archived" chip, and opens read-only with a Restore control. The legend gains "Archived (N)" to show them in a collapsed group. A daily sweep archives plain chats idle for more than `session_archive_idle_days` (default 30; pinned chats are exempt; chats in a space are archived, never deleted), and `session_delete_archived_days` (default 0 = never) hard-deletes chats archived longer than that. `GET /api/sessions?archived=1`, `PATCH /api/sessions/{id} {archived}`, `POST /api/sessions/archive-idle {days, space_id, dry_run}`. Migration v34 adds `archived_at`.

- feat(settings): the Storage tab owns the archive: both knobs live there, with "Archive idle chats now" and "Delete archived chats older than…" as preview-then-confirm actions, and the sweep counters in the ledger. `POST /api/storage/prune-archived {days, dry_run}` (dry run by default).

- fix(backup): two backup directories, both reported and rotated. Besides `data/backups`, an older `data/.backups` still receives a snapshot on every deploy; it held 2.7 GB in three naming schemes, one of which (`sessions.20260825T183703Z.db`) rotation did not even recognise. The Storage tab shows it as "Pre-deploy copies (legacy)" with its own preview-then-confirm rotation; `GET /api/storage` reports `legacy_backups`, and `POST /api/storage/backups/rotate` takes `dir: primary | legacy`.

- fix(health): `sessions_active` in `/api/health` now counts sessions doing work (mid-turn by the state machine, canaries excluded), and `sessions_loaded` carries the old number, sessions held in memory. The old field read nine on an idle box.

- fix(sessions): the session purge sees the whole table and deletes only what it says. `POST /api/sessions/purge` used to read the 1,000 most recently updated rows, so on a box with 1,032 sessions the 32 oldest chats were invisible to it, and it deleted every session type, pinned chats included. It now selects plain chats only (never pinned, never in a space; automation sessions keep their own retention), takes `dry_run: true` and answers with the same counts, sample titles and skipped tallies the real run uses.

- feat(settings): a Storage tab. Sessions by type, database size with what a compaction would reclaim, the backup snapshots with a preview-then-confirm rotation, a Compact database action, and the Session Cleanup section moved here from Environment & network with its Preview now asking the server. `GET /api/storage`, `POST /api/storage/backups/rotate`, `POST /api/storage/optimize`. Compaction checkpoints the write-ahead log so the file actually shrinks.

- fix(backup): snapshot rotation counts every snapshot it ever wrote. Three generations of the backup script named files three ways, and rotation only saw the newest naming, so 2.7 GB of copies of a 168 MB database had accumulated. `scripts/backup.py --dry-run` lists what rotation would remove.

- fix(theme): the State timeline's graph gets its colours back. Every state box and label had gone black: the palette tokens became `color-mix()` expressions, the browser reports those as `color(srgb 0.81 0.85 0.86)`, and the token-to-hex helper read the 0–1 floats as 0–255. The helper now resolves any colour syntax through a canvas. A second fault found on the way: under reduced-motion the same helper read a colour mid-transition, which collapsed the whole palette (sigil and editor included) for those users.

- fix(status): the "Scout: N tools · 1.2s" line no longer lingers over the chat on a phone. On touch the status line is a strip floating above the composer, and the only thing that cleared the scout summary was the answer's first streamed token — so a turn that ran tool calls first kept the box over the transcript for all of them, and a turn that ended without text kept it for good. It now clears the moment the turn moves on (first tool call, first token, or the turn ending), and on touch any status message that states a finished fact fades after six seconds, while in-progress ones (retries, compaction, "Stopping…") stay until their own event ends them. Desktop is unchanged.

- fix(ui): the app's own words catch up with the interface pass. `/help` and the voice-input notices point at Settings → Integrations → Voice Input and at Explorer → Automation → Jobs / Capabilities → Skills, with the folder emoji gone; the Ctrl+K palette names every Explorer pane as `Open Explorer → Group → Tab` using the labels the panel shows (Servers, Self-checks, Goals, Learning), derived from the Explorer's own table so the two cannot drift again, while `canary`, `telos`, `mcp` and `adaptive` still find them; and closing Settings with unsaved edits asks with the app's own dialog (Keep editing / Discard) instead of the browser's confirm box.

- feat(ui): the interface pass — three phases over the whole front end, aimed at someone who did not write it. Nothing about agent behaviour changed:
  - **Accessibility as a floor, not a polish layer.** Landmarks, a page `h1`, a skip link, a global `[hidden]` rule, and `prefers-reduced-motion` honoured; every icon-only control named; disclosures, tool rows, sidebar rows, status controls and the Explorer/Timeline operable from a keyboard with a visible focus ring and 24px targets. New `static/js/a11y.js` gives the app real modal overlays (focus trap, `inert` behind, restore on close, Escape) and one live region, so what a sighted user reads off the screen — a turn finishing, a job failing, a save landing — is announced instead of silent. Global shortcuts stop firing through an open dialog, and the offscreen sidebar drops its `inert` when the drawer takes over.
  - **One button system, one icon language, honest contrast.** `static/css/buttons.css` replaces inline `style=` attributes and the near-duplicate `.btn-sm` variants with one set of classes; `static/js/icons.js` replaces the text glyphs and per-panel SVG snippets with one inline set every panel and the status bar draw from. Text contrast raised to AA, one pulse keyframe, one transition vocabulary.
  - **Light theme.** A complete light palette in `tokens.css`, every colour in every stylesheet resolved through a token (canvas, editor and diagram included), a System / Dark / Light control under Settings → Providers & models → Appearance (browser-local — it belongs to the device, not the agent), and `theme-boot.js` applying the stored choice before first paint so there is no flash.
  - **Explorer: two levels of navigation.** Nine flat tabs become five groups — **Files / Knowledge / Capabilities / Automation / Self-tuning** — with sub-tabs underneath. Leaf keys are unchanged, so every existing deep link still lands. Every empty state now says what to do next instead of showing an empty box, and Explorer failures reach the user instead of the console.
  - **Settings: grouped by what you came to do.** One 21-section "General" tab plus four specialists become six — **Providers & models / Agent behaviour / Autonomy & idle work / Tools & safety / Integrations / Environment & network** — with a search that spans all six at once. Every field appears exactly once (the grouping is a `tab` key on each section, so a field cannot be listed twice or dropped), old deep links alias to the new tabs, and controls now show their default, range and unit read from `GET /api/settings/schema` instead of a hardcoded copy in the browser.
  - **User-facing names, internal terms kept.** Scheduled, Large-input runs, Self-checks, Servers, Learning, Goals — with cron, RLM, canary, MCP, adaptive and Telos riding along in tooltips and section titles so search, the docs and the code still connect.
  - **Editing that cannot silently lose work.** The workspace, skill and memory editors send the mtime they read back as `base_mtime`; a save whose base is stale is refused with a 409 and the user is offered Reload or Overwrite instead of one of the two versions vanishing. All three ask before discarding unsaved edits, as does Settings.
  - **Destructive actions ask once, in one voice.** Every `window.confirm()` and `alert()` is gone: one shared confirm dialog, one stacked toast primitive (`feedback.js`), and deleting a session is undoable from the toast rather than a point of no return.
  - **Transcripts that read like they streamed.** Replay draws a tool round the way the live stream drew it; a turn lost in an SSE gap says so; the empty streaming bubble is cleared on every turn-ending path; earlier messages load by prepending (`before_id`) instead of re-rendering the whole transcript; every answer carries a chip naming the model that wrote it and what it cost.
  - **Sessions, workers, palette.** One worker card per fan-out with a strip that updates in place and a revive on dead chips; a worker or RLM view says which session it belongs to; the session list reaches past the 500 most recent instead of stopping there; memory browses at corpus scale (paged search, **Load more**, strong/good/weak labels); and the Ctrl+K palette runs commands as well as finding sessions.

- feat(ui): the mobile and tablet pass — the same interface on a phone, a tablet and a monitor, with the device tier decided by two questions instead of one (design notes: `docs/internals/web-client.md`):
  - **A real tablet tier.** "Mobile" used to mean one thing, so a 1180px iPad in landscape got the phone layout. Layout now follows the viewport (under 900px: drawer, sheets, full-screen Explorer) and input follows the pointer (finger: 44px targets, 16px inputs, no hover-only controls), independently. A landscape iPad keeps the docked sidebar and puts the Explorer beside the conversation, at touch sizes. Recognising an iPad at all takes JavaScript — iPadOS desktop mode reports a Mac, `hover: hover` and `pointer: fine`, so no media query can see one.
  - **One menu per row instead of hover-revealed icons.** Sessions, spaces and Explorer files carry a single `⋯` control that opens an action sheet naming every action in words; on a phone the session header's title opens the same sheet rather than turning into a rename field under your thumb.
  - **The model picker is a bottom sheet below 900px** — full width, 48px rows, a scrim and Escape — instead of a menu as wide as the longest model id hanging off a 19px badge.
  - **The file editor is a plain textarea on touch**, never Monaco: native selection handles, 16px text, and lines that wrap instead of demanding a horizontal scroll nobody can aim at.
  - **Keyboard sending that suits the device.** Ctrl+Enter / Cmd+Enter always sends, on every device — the answer for a tablet with a keyboard. Enter sends under a mouse and adds a line on touch (where it is the on-screen keyboard's newline key), and the default is flippable under Settings → Providers & models → *This browser*, whose two rows are stored in that browser and never synced. The composer's hint always names what the keys actually do.
  - **The worker strip is one line below 900px** — `N workers · M RLM · K finished`, tap for the worker sheet — instead of three rows of 16px chips over the transcript.
  - **Targets, type and edges.** Every control reaches at least 28px (44 where it is a row's primary action) and no text sits under 11px; every input is at least 16px, below which iOS zooms the page on focus and never zooms back. Code blocks get a header strip so the copy button stops covering the first line; code and tables that really do overflow fade at the edge that continues; scrolling tab strips say which way they scroll; the jump-to-bottom button moved off the text it was covering.
  - **`mobile.css` is now `compact.css` (width) and `touch.css` (pointer)**, each gated on its own `<link media>`, and `isMobile()` is gone in favour of `isCompact()` / `isTouch()`. `tools/ui-gate/` drives the app across seven viewports and asserts the tier contracts, the touch floors and an unchanged desktop layout, and `docs/mobile-device-checklist.md` covers what a headless browser cannot: the iOS keyboard, native text selection, rotation and VoiceOver.

- feat(api): the endpoints and parameters the interface pass needed (all documented in `docs/api.md`): `GET /api/settings/schema` (per-field type, default, min/max, step, unit, restart, locked); `limit`/`offset` with `returned`/`has_more` on `GET /api/memory/search`; `PUT /api/memory/files/{name}` (`content`, optional `base_mtime`, 409 `changed_on_disk` + current `mtime`) with `mtime` added to the matching GET; `total`/`has_more` on `GET /api/sessions`; `before_id`/`has_more` on `GET /api/sessions/{id}`; `POST /api/jobs/{name}/run` and `POST /api/jobs/{name}/validate`; the same `base_mtime` → 409 contract on `PUT /workspace/{path}` and `PUT /api/skills/{name}`, with the read side handing back `X-File-Mtime` (workspace) or `mtime` (skills); and a `path` field on `POST /api/upload` so an upload lands in the folder the Explorer is showing.

- feat(spaces): **Spaces** — named/colored groups of long-lived sessions that share their work (migration v33; guide: `docs/guides/spaces.md`):
  - **Grouping + never-roll-off**: spaces render above the time buckets in the sidebar (collapsible, per-space `+`/gear/delete); their sessions leave the buckets entirely and are unioned into the sidebar payload past the 500-row window. Move sessions in/out via the row's space picker — membership changes never touch recency. Space chips in session search and the Ctrl+K palette.
  - **Per-space directives**: `data/agent/spaces/<slug>/{SOUL,RULES,SESSIONS}.md` override the defaults per file (undefined files fall back); modal editor with read-only default → Customize → Revert; compiler and scout both resolve through `core/spaces.directive_path`. Birthdate stays on the default SOUL.md.
  - **Workspace home, not a jail**: bash cwd/HOME and relative paths default to `data/workspace/spaces/<slug>/` via a new `WORKSPACE_HOME` ContextVar; read/write roots stay global (a real `workspace_override` sandbox always wins), and reads prefer the candidate that exists so bare names still find global files. Workers inherit the parent's space + home.
  - **Memory**: writes auto-route to `pernix.space.<slug>.*` (explicit names stay a contract; the routing Jaccard step gained a bucket guard so `space.alpha.research` can never map onto `space.beta.research`); searches from space sessions surface space entries first by ORDER ONLY — the documented score scale is untouched, with a noise-floor guard; consolidation/reroute sweeps never cross bucket boundaries; Memory tab badges space files. `MemoryStore.delete_file` added for cascade deletes.
  - **Cron**: `space_id` rides job `extra_meta`; each firing creates its fresh `Cron:` session inside the space; `schedule_job` inherits the calling session's space (`space_id="none"` opts out); Jobs tab shows a space chip and the add form a space picker. Fixed en route: pinned `Cron:` sessions are now excluded from the 7-day title sweep, and `/api/sessions/purge` skips space sessions.
  - **Shared kernel per space**: all member sessions share one REPL kernel (`space-<id>` registry key, cwd pinned to the space home); the mid-turn eviction/reap guard widens to "any member processing"; member-session delete never purges it, space cascade-delete does; the busy error names the space.
  - **RLM**: runs from space sessions mint under `spaces/<slug>/rlm/`, are listed per-space (`/api/rlm/runs?space_id=`), and `continue_from` resolves run dirs through the registry row so sibling sessions can continue them.

- feat(mcp): native MCP client — external tool servers as first-class Pernix tools (a3ab8e5, ec725d6, 62a87f1, 2caf73d; design: `docs/dev/mcp-integration-plan.md`, guide: `docs/mcp.md`):
  - **Registry-first architecture** on the official `mcp` SDK 2.1.\*: each server configured in `data/mcp_servers.json` (the ecosystem-standard `mcpServers` shape — Claude Code / Cursor / VS Code configs paste verbatim) gets a supervised connection on the main event loop, and its tools register as `mcp_<server>_<tool>` with `source="mcp"` — scout curation, the dangerous-tool gate, per-tool health metrics, post-mortems and Candor grading apply unchanged. Enabled by default but fully inert with zero servers configured (configuring one is the opt-in); `mcp_enabled` toggles hot both ways.
  - **Transports**: stdio (local subprocess; `mcp_stdio_enabled=false` = remote-only mode, the supply-chain valve), Streamable HTTP, and the deprecated legacy SSE. Lifecycle: background connect that never blocks boot, exponential-backoff reconnect with one-shot incident notifications, idle stdio suspend/resume (child reaped after `mcp_idle_seconds`, respawned transparently on the next call — validated live), `listChanged` + periodic tool refresh with diff-aware re-registration.
  - **Safety posture**: default level `caution` per tool, server-sent `destructiveHint` escalates to `dangerous` (annotations can never loosen), per-server `safety`/`timeout`/`tool_allowlist` overrides, canary sessions denied MCP outright, descriptions length-capped and provenance-prefixed `[MCP:<server>]` (untrusted text headed for the system prompt), secrets only as `${VAR}` references into `.env` — literal-looking tokens in config are rejected.
  - **Surfaces**: Explorer → Capabilities → Servers (MCP) (status dots, paste-import with dry-run Test, per-row edit/reload/toggle/remove), Settings → Integrations → MCP Servers, REST `/api/mcp/*` (list/add/delete/toggle/reload/test), and four agent tools (`mcp_add_server` is dangerous-gated; `mcp_list_servers`, `mcp_reload_server`, `mcp_remove_server`). Private-to-the-box deployment patterns (sibling container on the compose network with no published ports; host process bound to the docker gateway IP) documented in `docs/mcp.md` after live validation.
  - **Core changes**: `ToolRegistry.unregister()`; safety overrides in `tools.json` are now retained for tools that register after boot (previously a user's override on any late registrant was silently dropped on the next save); the no-scout fallback surface excludes `source="mcp"` tools except ones the session already used.
  - Fixes from live burn-in: the refresh sweep no longer races the idle reaper (spurious "Connection closed" warning every suspend), and anyio TaskGroup wrappers are unwrapped to the real cause everywhere a person sees an error ("All connection attempts failed", not "unhandled errors in a TaskGroup").

- fix(scheduling,memory): two bugs from the 2026-08-31 curiosity-drive session audit (1537815d412f):
  - **Phantom coalesced catch-ups.** The boot-time missed-run counter rebuilt the cron grid with a hardcoded UTC timezone while live job triggers run on the container's local zone (`from_crontab` with no tz → America/Chicago) — five hours of every day the two grids disagreed, and any restart in that gap dispatched a catch-up for a slot that had already fired (the 17:00 UTC slot completed at 17:03; an 18:30 UTC deploy restart "coalesced" a nonexistent 18:00 UTC slot, spawning a spurious run and run-numbering confusion). `_count_missed_fires` now counts on the job's own live `job.trigger`, so the grids can never disagree, DST included.
  - **remember() honored over content gravity.** `_resolve_file_name` filtered its exact-match candidates to `entry_count > 0`, making an explicitly named EMPTY file invisible — its first entry could never land by name — and the content-dominance step then hijacked the write to whichever file already held similar entries, self-reinforcing with every mis-route (pernix.findings sat at 0 entries while 4 runs' findings were silently diverted to a look-alike file; reflect burned two retries on it). Exact match now sees all known files (explicit writes to archived files still flow to add_entry's revive path), and a valid explicit name that matches nothing existing is honored instead of content-routed: an explicit target is a contract, not a hint. Content-based placement still decides when no file is named.

- feat(refine,skills,snooze,db): skill self-healing — when a skill fails and a session finds a workaround, the loop that folds the workaround back into the skill now actually fires (audit of session 83dc931a8596: youtube-whisper failed 5×, the CPU-on-aibox workaround succeeded, and the skill was only fixed because the user asked — zero refine proposals had ever reached the table). Five pieces:
  - **Multi-signal skill attribution.** `_identify_active_skill` detected only `load_skill` tool calls, but scout-planned sessions run skill scripts directly and never call it — so refine silently dropped every proposal it wrote. It now also attributes from `skills/<name>/` path references across assistant/tool/scout messages and from registered skill names in the scout plan, validated against the registry (a stray path can't route a proposal to a nonexistent skill). `load_skill` still outranks both.
  - **Re-armable refine watermarks** (migration v32). `refined:{sid}` stores the max message id seen at selection instead of a timestamp — a session that grows (ask_user answered, resumed, workaround found late) qualifies again instead of being one-shot graded mid-story. Legacy ISO watermarks convert to "processed up to now" so nothing re-refines on deploy. Refine's selection also stops treating `awaiting_user`/`awaiting_workers` as idle: mid-task pauses aren't finished stories, and the re-arm makes waiting safe. Proposals dedupe against existing non-rejected rows per skill so re-refines can't mint the same change twice.
  - **Failure-arc evidence.** Refine's prompt saw only the last 8000 transcript chars — a long session's failure→workaround arc never fit. Failing tool/job results (Error prefix, `state=failed`, tracebacks, non-zero `exit=`) are now extracted chronologically (re-polls deduplicated), paired with the assistant intent that preceded them, and pinned into the prompt with a machine-signal line ("N failures then success — the workaround is what the skill is missing") alongside the original task and the tail.
  - **Veto-window auto-apply** (snooze Activity 13b). Pending SKILL.md proposals older than `skill_proposal_auto_apply_after_hours` (24; 0 disables) are machine-validated (skill exists + enabled, change ≤ 4000 chars, confidence ≥ 0.6) and applied with a timestamped backup under `data/skill_backups/<skill>/`, status `auto_applied`, day-capped (`skill_proposal_max_auto_applies_per_day`, 5), idle-only, announced as a notification — the SKILL.md counterpart of adaptive's `auto_approve_stale_proposals`, per the snooze philosophy: veto windows + rollback, not approval gates. Proposals whose target skill is gone archive instead of parking forever.
  - **Skill-change → memory re-validation** (snooze Activity 13c). Each enabled skill's SKILL.md + scripts are content-hashed (`skill_content_hash:{name}`, baseline stored silently on first sight); when one changes — auto-apply, in-session agent edit, or human edit on disk — memory entries mentioning the skill are cited into `memory_stale` dream hypotheses (hash-guarded refs, capped 6/skill, deduped against pending, one skill per cycle) so stale claims like "the script lacks a CPU flag" get re-judged by the dream validator instead of contradicting the fixed skill for months.

- fix(scout): four fixes from the 2026-08-28 scout audit (506 runs / 8 days aggregated, 215 graded turns joined, live reproduction before every change):
  - **Hint echo un-starved.** `used_hints` was echoed in 0 of 215 organic graded turns while the mechanism provably works (a live repro with a dead-center-relevant hint bumped the signal immediately) — and routing hints have no other usage channel, so the v3.1 usage sweep would have retired every non-exempt hint at epoch+45d regardless of value. The echo criterion is softened from "actually shaped this plan / do not echo every hint" to "influenced the plan, even partially", the field guidance explains the retention stake, and `used_hints` joins the schema's required list ([] allowed) so emission is the norm and echo density is measurable.
  - **scout.done becomes auditable.** The event (and the persisted scout message) now carries `used_hints`, `task_type`, `execution_mode`, `viability`, `scout_rounds` (new report field — LLM rounds the loop actually spent), and prompt/completion token counts. The audit had to reconstruct all of this from post-mortems; echo density and latency drivers (searching vs prompt size) were unmeasurable per-turn.
  - **Worker-resume turns reuse the suspended turn's scout report** (same mechanism as ask_user answers). Resume scouts measured 17–25s each and every sampled plan restated the action the resume message already mandates. Gap-1 resumes (parent already idle) still scout fresh by design.
  - **The report cache is deleted.** 0 hits in 506 live runs (exact-string key + session + phase + 25%-utilization bucket, 5-minute TTL — real traffic never repeats that precisely). The hint-usage counting that lived in its write path moves to the fresh-report acceptance seams unchanged; `from_cache` stays on the report (always False) for downstream compat; the 2026-08-07 cache-on-retries regression test retires with the cache it pinned.
...
- feat(adaptive,dream,telos): the value redesign — content gates, a usefulness signal, self-curation, and the TELOS carve, from the 2026-08-27 live audit (21 of 24 policy slots held Dream narrative complaints auto-approved unread; ~2,300 tokens of adaptive content in every turn, uncapped; no per-entry benefit signal anywhere; TELOS 89% question abandonment with a goal tree of one root node). No migrations. The pieces:
  ...
  - **The usefulness signal.** Rendered entries carry ids; scout echoes `used_hints` (counted once at the fresh-report seam), reflect cites `cited_policies` against a new id-carrying evidence section; both land as `adaptive_entry` rows in `scout_signals` (no migration) and surface as usage badges in the Explorer's Self-tuning → Learning (Adaptive) tab.
